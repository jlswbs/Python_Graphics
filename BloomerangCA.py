...
pygame.display.set_caption("Bloomerang Cellular Automaton")
...
while running:
    ...
    alive = (grid == 1)
    surface_array[alive] = [255, 255, 255]

    fading = (grid >= 2)
    if np.any(fading):
        t = (grid[fading] - 2) / (STATES - 2)

        surface_array[fading, 0] = (255 * (1 - t)).astype(np.uint8)
        surface_array[fading, 1] = (120 * (1 - t)).astype(np.uint8)
        surface_array[fading, 2] = (255 * t).astype(np.uint8)
    ...
    pygame.display.set_caption(
        f"Bloomerang CA - Gen: {generation} {'[PAUSED]' if paused else ''}"
    )

...
pygame.display.set_caption("Bugs Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        birth = (grid == 0) & ((neighbors == 2) | (neighbors == 3) | (neighbors == 4))
        new_grid[birth] = 1
    ...
    surface_array[grid == 2] = [140, 240, 70]
    surface_array[grid == 3] = [100, 200, 40]
    surface_array[grid == 4] = [80, 160, 30]
    surface_array[grid == 5] = [60, 120, 25]
    surface_array[grid == 6] = [40, 90, 20]
    surface_array[grid == 7] = [25, 60, 15]
    ...
    pygame.display.set_caption(
        f"Bugs CA - Gen: {generation} {'[PAUSED]' if paused else ''}"
    )
    pygame.display.flip()
    clock.tick(FPS)

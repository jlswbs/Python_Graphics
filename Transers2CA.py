# Transers II Cellular Automaton
...
pygame.display.set_caption("Transers II Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        birth = (grid == 0) & ((neighbors == 2) | (neighbors == 6))
        new_grid[birth] = 1
        ...
        dying_start = (grid == 1) & ~survive
        new_grid[dying_start] = 2
    ...
    surface_array[grid == 1] = [100, 150, 255]
    surface_array[grid == 2] = [80, 120, 220]
    surface_array[grid == 3] = [60, 90, 185]
    surface_array[grid == 4] = [40, 60, 150]
    surface_array[grid == 5] = [20, 30, 100]
    ...
    pygame.display.set_caption(
        f"Transers II CA - Gen: {generation} {'[PAUSED]' if paused else ''}"
    )
    pygame.display.flip()
    clock.tick(FPS)

...
pygame.display.set_caption("Big Bang Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        survive = (grid == 1) & ((neighbors >= 2) & (neighbors <= 6))
        new_grid[survive] = 1
    ...
    surface_array[grid == 1] = [255, 255, 200]
    surface_array[grid == 2] = [255, 220, 80]
    surface_array[grid == 3] = [255, 160, 40]
    surface_array[grid == 4] = [255, 100, 20]
    surface_array[grid == 5] = [240, 60, 10]
    surface_array[grid == 6] = [200, 30, 5]
    surface_array[grid == 7] = [140, 10, 0]
    ...
    pygame.display.set_caption(
        f"Big Bang CA - Gen: {generation} {'[PAUSED]' if paused else ''}"
    )
    pygame.display.flip()
    clock.tick(FPS)

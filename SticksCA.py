...
pygame.display.set_caption("Sticks Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        survive = (grid == 1) & ((neighbors >= 3) & (neighbors <= 6))
        new_grid[survive] = 1
    ...
    surface_array[grid == 1] = [180, 255, 120]
    surface_array[grid == 2] = [140, 220, 80]
    surface_array[grid == 3] = [100, 180, 50]
    surface_array[grid == 4] = [70, 140, 30]
    surface_array[grid == 5] = [50, 100, 20]
    surface_array[grid == 6] = [30, 60, 15]
    ...
    pygame.display.set_caption(f"Sticks CA - Gen: {generation} {'[PAUSED]' if paused else ''}")
    pygame.display.flip()
    clock.tick(FPS)

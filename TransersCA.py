...
pygame.display.set_caption("Transers Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        survive = (grid == 1) & ((neighbors == 3) | (neighbors == 4) | (neighbors == 5))
        new_grid[survive] = 1
    ...
    surface_array[grid == 1] = [80, 255, 220]
    surface_array[grid == 2] = [60, 220, 180]
    surface_array[grid == 3] = [40, 170, 140]
    surface_array[grid == 4] = [25, 120, 100]
    surface_array[grid == 5] = [15, 70, 60]
    ...
    pygame.display.set_caption(f"Transers CA - Gen: {generation} {'[PAUSED]' if paused else ''}")
    pygame.display.flip()
    clock.tick(FPS)

...
pygame.display.set_caption("Blink Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        birth = (grid == 0) & ((neighbors == 2) | (neighbors == 3))
        new_grid[birth] = 1

        survive = (grid == 1) & (neighbors == 3)
        new_grid[survive] = 1
    ...
    surface_array[grid == 1] = [140, 220, 255]
    surface_array[grid == 2] = [110, 180, 240]
    surface_array[grid == 3] = [90, 140, 220]
    surface_array[grid == 4] = [70, 100, 190]
    surface_array[grid == 5] = [50, 70, 160]
    surface_array[grid == 6] = [35, 45, 120]
    surface_array[grid == 7] = [20, 25, 80]
    ...
    pygame.display.set_caption(f"Blink CA - Gen: {generation} {'[PAUSED]' if paused else ''}")
    pygame.display.flip()
    clock.tick(FPS)

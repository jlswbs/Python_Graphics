# DeJong attractor renderer (color gradient, centered with margin)
...
def render_dejong(a, b, c, d,
    ...
    for i in range(height):
        for j in range(width):
            h = 0.65 - 0.65 * norm[i, j]
            s = 1.0
            v = norm[i, j]
            r, g, b = colorsys.hsv_to_rgb(h, s, v)
            rgb[i, j] = (int(r * 255), int(g * 255), int(b * 255))

    image = Image.fromarray(rgb, mode='RGB').transpose(Image.FLIP_TOP_BOTTOM)

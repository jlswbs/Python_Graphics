...
pygame.display.set_caption("Fireworks Cellular Automaton")
...
while running:
    ...
    if not paused:
        ...
        survive = (grid == 1) & ((neighbors == 3) | (neighbors == 4))
        new_grid[survive] = 1
    ...
    surface_array[grid == 1] = [255, 240, 120]
    surface_array[grid == 2] = [255, 180, 60]
    surface_array[grid == 3] = [255, 120, 30]
    surface_array[grid == 4] = [255, 60, 20]
    surface_array[grid == 5] = [220, 30, 80]
    surface_array[grid == 6] = [180, 20, 120]
    surface_array[grid == 7] = [120, 10, 100]
    ...
    pygame.display.set_caption(f"Fireworks CA - Gen: {generation} {'[PAUSED]' if paused else ''}")
    pygame.display.flip()
    clock.tick(FPS)
